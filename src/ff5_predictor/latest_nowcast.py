...
from ff5_predictor.nowcast_engine import (
    NowcastTargetSpec,
    empty_nowcast_predictions,
    run_nowcast_engine,
    select_nowcast_columns,
)
...
def run_latest_nowcast(config: dict[str, Any]) -> LatestNowcastResult:
    ...
    return run_latest_nowcast_from_frames(ff5_df, market_df, config)


def run_latest_nowcast_from_frames(
    ff5_df: pd.DataFrame,
